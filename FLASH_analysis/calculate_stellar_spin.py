        ...
        if os.path.exists(single_pickle):
            ...
            for sink_id in sink_data.keys():
                ...
                if sink_id == list(sink_data.keys())[0]:
                    axs.flatten()[plot_it].plot(time.in_units('yr'), T_rot.in_units('day'), color=colors[Mach_labels.index(mach_lab)], linestyle=linestyle, label='$\mathcal{M}=$'+mach_lab)
                else:
                    axs.flatten()[plot_it].plot(time.in_units('yr'), T_rot.in_units('day'), color=colors[Mach_labels.index(mach_lab)], linestyle=linestyle)
        else:
            print("Couldn't open", single_pickle)
...
axs.flatten()[plot_it-1].set_ylim([0.6, 2])
axs.flatten()[plot_it-1].set_xlim([0, 10000])
