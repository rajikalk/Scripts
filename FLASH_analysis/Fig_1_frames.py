    ...
    for mach_val in mach_values:
        ...
        if os.path.exists(pickle_file) == False:
            ...
            if max_time[spin_values.index(spin_val)][mach_values.index(mach_val)] == None:
                ...
                end_time_string = str(max_time[spin_values.index(spin_val)][mach_values.index(mach_val)])
            cmd = ['python', '/home/kuruwira/Scripts/FLASH_analysis/movie_script.py', '/home/kuruwira/fast/Protostellar_spin/Flash_2023/Spin_'+spin_val+'/Single/Mach_'+mach_val+'/Lref_9/', './', '-pt', end_time_string, '-width', str(width), '-thickness', '500', '-no_quiv', '15', '-stdv', str(stdvel), '-ax', 'y']
            subprocess.Popen(cmd).wait()
