...
for spin_lab in Spin_labels:
    for mach_lab in Mach_labels:
        ...
        if os.path.exists(single_pickle):
            ...
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_orbit/L_tot, linestyle = '--', color='k', linewidth=2, label='Orbit')
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_in_gas/L_tot, linestyle = ':', color='k', linewidth=2, label='Gas')
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_sink_tot/L_tot, linestyle = '-', color='k', linewidth=2, label = 'Sink total')
            cit = -1
            for sink_id in L_sink.keys():
                cit = cit + 1
