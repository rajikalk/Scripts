...
for spin_lab in Spin_labels:
    for mach_lab in Mach_labels:
        ...
        if os.path.exists(single_pickle):
            ...
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_orbit/L_tot, linestyle = '--', color='k', linewidth=3, label='Orbit', alpha=0.25)
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_in_gas/L_tot, linestyle = ':', color='k', linewidth=3, label='Gas', alpha=0.25)
            axs.flatten()[plot_it].semilogy(Time_array - Time_array[0], L_sink_tot/L_tot, linestyle = '-', color='k', linewidth=3, label = 'Sink total', alpha=0.25)
            cit = -1
            form_times = []
            for sink_id in L_sink.keys():
                form_times.append(np.array(L_sink[sink_id]).T[0][0])
            sort_inds = np.argsort(form_times)
            for sink_id in np.array(list(L_sink.keys()))[sort_inds]:
                cit = cit + 1
