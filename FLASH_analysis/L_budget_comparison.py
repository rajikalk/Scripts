...
for spin_lab in Spin_labels:
    ...
    for mach_lab in Mach_labels:
        ...
        if os.path.exists(single_pickle):
            ...
            for sink_id in L_sink.keys():
                axs.flatten()[plot_it].semilogy(np.array(L_sink[sink_id]).T[0]-np.array(L_sink[sink_id]).T[0][0], np.array(L_sink[sink_id]).T[1].value, linestyle = mach_ls[Mach_labels.index(mach_lab)], color=colors[0], linewidth=1)
