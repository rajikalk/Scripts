...
if args.make_movie_pickles == 'True':
    ...
    for fn in yt.parallel_objects(usable_files, njobs=njobs):
        ...
        if make_pickle:
            ...
            proj_depth = yt.ProjectionPlot(ds, args.axis, ['z', 'Neg_z', 'dz', 'Neg_dz'], width=(args.plot_width,'au'), weight_field=None, data_source=region, method='mip', center=dd['Center_Position'].in_units('cm'))
            thickness = ((proj_depth.frb.data[('gas', 'Neg_z')].in_units('cm') + proj_depth.frb.data[('gas', 'Neg_dz')].in_units('cm')/2.) + (proj_depth.frb.data[('gas', 'z')].in_units('cm') + proj_depth.frb.data[('gas', 'dz')].in_units('cm')/2.))
            proj_dict = {}
