    ...
    for fn in yt.parallel_objects(usable_files, njobs=njobs):
        ...
        if make_pickle:
            ...
            for sto, field in yt.parallel_objects(proj_field_list, storage=proj_dict):
                ...
                if 'Relative_keplerian_velocity_wrt_primary' in field:
                    import pdb
                    pdb.set_trace()
    ...
    for pickle_file in pickle_files:
        ...
        if rank == rit:
            ...
            if os.path.isfile(file_name+'.jpg') == False:
                ...
                if 0 in cbar_lims:
                    plot = ax.pcolormesh(X_image, Y_image, image, cmap=cmap, vmin=cbar_lims[0], vmax=cbar_lims[1], rasterized=True, zorder=1)
                else:
