    ...
    for fn in yt.parallel_objects(usable_files, njobs=njobs):
        ...
        if make_pickle:
            ...
            if rank == proj_root_rank and size > 1:
                proj_dict[list(proj_dict.keys())[4]] = proj_dict[list(proj_dict.keys())[4]] - center_vel[0]
                if args.axis == 'z':
                    proj_dict[list(proj_dict.keys())[5]] = proj_dict[list(proj_dict.keys())[5]] - center_vel[1]
                else:
                    proj_dict[list(proj_dict.keys())[5]] = proj_dict[list(proj_dict.keys())[5]] - center_vel[2]
            
                velx, vely, velz = mym.get_quiver_arrays(0, 0, X_image, proj_dict[list(proj_dict.keys())[4]], proj_dict[list(proj_dict.keys())[5]], no_of_quivers=args.quiver_arrows)
                file = open(pickle_file, 'wb')
                
                pickle.dump((X_image, Y_image, Toomre_Q_magnetic, proj_dict[list(proj_dict.keys())[6]], proj_dict[list(proj_dict.keys())[7]], X_image_vel, Y_image_vel, velx, vely, part_info, time_val), file)
                file.close()
