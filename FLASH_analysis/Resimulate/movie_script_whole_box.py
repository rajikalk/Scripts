...
def parse_inputs():
    ...
    parser.add_argument("-stdv", "--standard_vel", help="what is the standard velocity you want to annotate?", type=float, default=2)
...
if args.make_movie_pickles == 'True':
    ...
    if args.use_all_files == 'False' and  args.plot_time == None:
        ...
        if args.make_movie_frames == 'False':
    ...
    dd = ds.all_data()
    center_vel = yt.YTArray([np.sum(dd['velx']*dd['cell_mass'])/np.sum(dd['cell_mass']), np.sum(dd['vely']*dd['cell_mass'])/np.sum(dd['cell_mass']), np.sum(dd['velz']*dd['cell_mass'])/np.sum(dd['cell_mass'])], 'cm/s')
    ...
    for fn in yt.parallel_objects(usable_files, njobs=njobs):
        ...
        if make_pickle:
            ...
            if rank == proj_root_rank and size > 1:
                ...
                if args.axis == 'z':
                    proj_dict[list(proj_dict.keys())[2]] = proj_dict[list(proj_dict.keys())[2]] - center_vel[1]
                else:
                    proj_dict[list(proj_dict.keys())[2]] = proj_dict[list(proj_dict.keys())[2]] - center_vel[2]
