...
for pick_file in pickle_files:
    ...
    axs.flatten()[0].plot(time_arr.in_units('yr'), mass_arr.in_units('msun'), label=label[pickle_files.index(pick_file)], color=proj_colours[cit])
    axs.flatten()[1].plot(time_arr.in_units('yr'), L_tot_arr.in_units('kg*m**2/s'), label=label[pickle_files.index(pick_file)], color=proj_colours[cit])
    axs.flatten()[2].plot(time_arr.in_units('yr'), L_spec.in_units('m**2/s'), label=label[pickle_files.index(pick_file)], color=proj_colours[cit])
    ...
    axs.flatten()[2].set_xlabel('Time (yr)')
    axs.flatten()[0].legend()
    plt.savefig("resolution_study_linear.pdf", bbox_inches='tight', pad_inches=0.02)
