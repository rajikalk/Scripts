...
if len(sim_dirs) > 0:
    ...
    sim_dirs = [x[0] for x in os.walk('/home/100/rlk100/rlk/Protostellar_spin/')]
    ...
    for sim_dir in sim_dirs:
        if os.path.exists(sim_dir+'/sinks_evol.dat') == True and 'Binary' not in sim_dir:
            ...
            if rank == rit:
                pickle_name = "_".join(sim_dir.split('Protostellar_spin/')[-1].split('/')) + '.pkl'
                if update == False and os.path.exists(pickle_name) == False:
                    proj_run_line = 'python3 /home/100/rlk100/Scripts/FLASH_analysis/read_sink_evol.py '+sim_dir+'/sinks_evol.dat /scratch/ek9/rlk100/Sink_evol_pickles/'+pickle_name
