...
for spin_val in spin_values:
    for mach_val in mach_values:
        plot_it = -1
        plt.clf()
        fig, axs = plt.subplots(ncols=2, nrows=n_frames, figsize=(0.59*two_col_width, page_height), sharex=True, sharey=True)
        ...
        fig.suptitle("$\Omega t_{ff}$="+spin_val+", $\mathcal{M}$="+mach_val, y=0.895)
        ...
        for plot_time in plot_times:
            ...
            if plot_it == len(plot_times)-2:
                ...
                cbar_ax = fig.add_axes([0.125, 0.095, 0.38, 0.015])
                ...
                cbar_ax = fig.add_axes([0.51, 0.095, 0.38, 0.015])
                cbar = fig.colorbar(plot, cax=cbar_ax, orientation='horizontal')
