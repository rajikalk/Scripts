...
axs.flatten()[0].set_ylabel("$\\frac{\Delta L}{L_{pre}}$ (%)")
...
axs.flatten()[1].set_xlabel('Initial cloud spin ($\Omega t_{ff}$)', labelpad=-0.2)
axs.flatten()[1].set_ylabel("$\\frac{\Delta h}{h_{pre}}$ (%)", labelpad=-1)
