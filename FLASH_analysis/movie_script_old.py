...
def get_files(path, args):
    ...
    if args.yt_proj == False:
        if args.axis == "xz":
            ...
            type = "slice"
        source_directory = sorted(glob.glob(path + 'SPIN_' + type + '*'))[:-1]
    else:
        source_directory = sorted(glob.glob(path + 'SPIN_hdf5_plt_cnt*'))[:-1]
...
def main():
    ...
    if args.yt_proj == False and args.image_center != 0:
        sim_files = sorted(glob.glob(path + 'SPIN_hdf5_plt_cnt*'))
