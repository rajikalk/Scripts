...
def parse_inputs():
    ...
    parser.add_argument("-wf", "--weight_field", help="Do you want to have a weighted projection plot?", type=str, default='cell_volume')
...
def main():
    ...
    if args.yt_proj:
        ...
        for file_int in range(len(usable_files)):
            ...
            if os.path.isfile(pickle_file) == False:
                print("PICKLE:", pickle_file,"DOESN'T EXIST. MAKING PROJECTION  FOR FRAME", frames[file_int])
                ...
                print("center_vel =", myf.get_center_vel(), "for", ds)
                if args.axis == 'xy':
    ...
    for frame_val in range(len(frames)):
        ...
        if args.pickle_dump == False:
            print("using pickle_file", pickle_file)
            ...
            print('Created frame', (frames[frame_val]+1), 'of', no_frames, 'at time of', str(time_val), 'to save_dir:', file_name + '.eps')
            #del image
            #del magx
            #del magy
            #del velx
