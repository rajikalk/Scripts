...
f = open('angular_momentum_conservation.txt','r+')
...
for pf in ts:
    ...
    Lp = ((L1[0] + L2[0])**2. + (L1[1] + L2[1])**2. + (L1[2] + L2[2])**2.)**(0.5)
    L_p.append(Lp)
    ...
    L1_l = angular_momentum(rel_l_err1, p1)
    L1_h = angular_momentum(rel_h_err1, p1)
    L2_l = angular_momentum(rel_l_err2, p2)
    L2_h = angular_momentum(rel_h_err2, p2)
    Lp_l = ((L1_l[0] + L2_l[0])**2. + (L1_l[1] + L2_l[1])**2. + (L1_l[2] + L2_l[2])**2.)**(0.5)
    Lp_h = ((L1_h[0] + L2_h[0])**2. + (L1_h[1] + L2_h[1])**2. + (L1_h[2] + L2_h[2])**2.)**(0.5)
    ...
    for x in range(dim):
        for y in range(dim):
            for z in range(dim):
                ...
                L_l = (angular_momentum(rpos_l, p))
                L_h = (angular_momentum(rpos_h, p))
                L_x = L_x + L[0]
                L_y = L_y + L[1]
                L_z = L_z + L[2]
                L_x_l = L_x_l + L_l[0]
    ...
    Lg = (L_x**2. + L_y**2. + L_z**2.)**0.5
    Lg_l = (L_x_l**2. + L_y_l**2. + L_z_l**2.)**0.5
    Lg_h = (L_x_h**2. + L_y_h**2. + L_z_h**2.)**0.5
    if Lg_l > Lg_h:
        temp = Lg_l
        Lg_l = Lg_h
        Lg_h = temp
    ...
    Lt = (L_x**2. + L_y**2. + L_z**2.)**0.5
    Lt_l = ((L1_l[0] + L2_l[0] + L_x_l)**2. + (L1_l[1] + L2_l[1] + L_y_l)**2. + (L1_l[2] + L2_l[2] + L_z_l)**2.)**0.5
    Lt_l = Lt - Lt_l
    Lt_h = ((L1_h[0] + L2_h[0] + L_x_h)**2. + (L1_h[1] + L2_h[1] + L_y_h)**2. + (L1_h[2] + L2_h[2] + L_z_h)**2.)**0.5
    Lt_h = Lt_h - Lt
    L_tot.append(Lt)
    it = it + 1
    
    f.write(str(time_val) + ',' + str(Lt) + ',' + str(Lp) + ',' + str(Lg) + ',' + str(Lt_l) + ',' + str(Lt_h) + '\n')
