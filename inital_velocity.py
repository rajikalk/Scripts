...
for z in range(dim):
    z_pos.append(z-128) #append z position. Take away 128 to have it relative to orbital plane.
    radius = distance([x_pos*gl,y_pos*gl,z*gl],pf.domain_center*pf["cm"])
...
top.plot(z_pos, kep_v)
#top.set_ylim([0.0, 0.15])
#top.set_xlim([0.0, 0.1])
top.set_title('Calculated Keplerian (x = ' + str(x_pos) + ', y = ' + str(y_pos) + ')')
top.set_ylabel('Keplerian Velocity ($cms^{-1}$)')
middle = fig.add_subplot(312, sharex=top)
middle.plot(z_pos, vel)
#middle.set_ylim([0.0, 0.25])
middle.set_title('Initial velocity (x = ' + str(x_pos) + ', y = ' + str(y_pos) + ')')
middle.set_ylabel('Velocity ($cms^{-1}$)')
bot = fig.add_subplot(313, sharex=top)
bot.plot(z_pos, rel_v)
#bot.set_ylim([0.0, 0.25])
bot.set_title('Relative velocity (x = ' + str(x_pos) + ', y = ' + str(y_pos) + ')')
bot.set_xlabel('z position (grid units)')
