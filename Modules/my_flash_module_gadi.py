...
def generate_frame_times(files, dt, start_time=0, presink_frames=25, end_time=None, form_time=None):
    ...
    if end_time != None:
        ...
        part_file=file[:-12] + 'part' + file[-5:]
        f = h5py.File(part_file, 'r')
        max_time = (f['real scalars'][0][-1]*yt.units.s).in_units('yr').value - sink_form_time
        f.close()
...
def annotate_particles(axis, particle_position, accretion_rad, limits, annotate_field=None, field_symbol="M", units=None, particle_tags=None, lw=1.5, zorder=4, split_threshold = 4):
    ...
    if annotate_field is not None:
        if len(particle_tags) > split_threshold:
            ...
            while split_it < len(particle_tags):
                sub_strings.append(",".join(p_t.split(',')[split_it-split_threshold:split_it])+",")
                sub_colors.append((['white'] + rainbow_text_colors[2*(split_it-split_threshold)+1:2*split_it+1]))
                split_it = split_it + split_threshold
            sub_strings.append(",".join(p_t.split(',')[split_it-split_threshold:]))
            sub_colors.append((['white'] + rainbow_text_colors[2*(split_it-split_threshold)+1:]))
            ...
            text_height = bbox_text.height*1.1
