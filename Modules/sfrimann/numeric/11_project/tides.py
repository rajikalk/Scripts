...
def read_july():
  ...
  for ttime, ttide in generator(open('./tides/july/list')):
    time.extend(ttime)
    tide.extend(ttide)
...
if __name__ == '__main__':
  ...
  hyphigh   = np.log([10 ,10 ,10  ,30  ])
  hyplow    = np.log([0.5,0.1,0.01,0.5])
  sigmahigh = 1
  sigmalow  = 0.001
  ...
  nlmlarr    = np.array(nlmlarr)
  hypdictarr = np.array(hypdictarr)
  np.savez('tideheight3', nlmlarr=nlmlarr, hypdictarr=hypdictarr)
  
  npzfile = np.load('tideheight3.npz')
  nlmlarr = npzfile['nlmlarr']
  hypdictarr = npzfile['hypdictarr']
  ...
  print '############################ Model 3 - Period with Decay ############################'
  print '                          Number of trials = %3i' % ntrial
  print '             Number of unique convergences = %3i' % len(u)
  print 'Best log marginal likelihood for the model = %.5f' % -nlml
  print '                                 Best mean = %.5f' % hypdict['mean']
  ...
  hyphigh   = np.log([30 ,10 ,10  ,30 ,10  ,30  ])
  hyplow    = np.log([0.5,0.1,0.01,0.5,0.01,0.5])
  sigmahigh = 1
  sigmalow  = 0.001
  ...
  nlmlarr    = np.array(nlmlarr)
  hypdictarr = np.array(hypdictarr)
  np.savez('tideheight4', nlmlarr=nlmlarr, hypdictarr=hypdictarr)
  
  npzfile = np.load('tideheight4.npz')
  nlmlarr = npzfile['nlmlarr']
  hypdictarr = npzfile['hypdictarr']
  ...
  print '############################ Model 4 - Double Periodic with Decay ############################'
  print '                          Number of trials = %3i' % ntrial
  print '             Number of unique convergences = %3i' % len(u)
  print 'Best log marginal likelihood for the model = %.5f' % -nlml
  print '                                 Best mean = %.5f' % hypdict['mean']
