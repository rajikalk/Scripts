...
if args.read_particle_file == 'True':
    ...
    if os.path.exists(pickle_file):
        try:
            ...
            particle_data, sink_form_time, init_line_counter = pickle.load(file_open)
            file_open.close()
        if 'accelx' in particle_data.keys():
            ...
            if line_counter >= init_line_counter:
                if row[0][0] != '[':
                    ...
                    if time_val not in particle_data['time']:
                        ...
                        np.vstack((particle_data['posx'].T, [np.nan, np.nan]))
                        np.vstack((particle_data['posy'].T, [np.nan, np.nan]))
                        np.append(particle_data['posy'].append([np.nan, np.nan])
                        np.append(particle_data['posz'].append([np.nan, np.nan])
                        np.append(particle_data['velx'].append([np.nan, np.nan])
                        np.append(particle_data['vely'].append([np.nan, np.nan])
                        np.append(particle_data['velz'].append([np.nan, np.nan])
