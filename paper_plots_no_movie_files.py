...
def parse_inputs():
    ...
    parser.add_argument("-calc_l_orb", "--calculate_orbital_momentum", help="do you want to orbital momentum", type=str, default="False")
                ...
                if row[0][0] != '[':
                    ...
                    if time_val not in particle_data['time']:
                        try:
                            ...
                            particle_data['anglx'] = np.vstack((particle_data['anglx'].T, [np.nan, np.nan])).T
                            particle_data['angly'] = np.vstack((particle_data['angly'].T, [np.nan, np.nan])).T
                            particle_data['anglz'] = np.vstack((particle_data['anglz'].T, [np.nan, np.nan])).T
...
if args.calculate_eccentricity == 'True':
    ...
    if calculate_e == True:
        ...
        V_CoM_x = np.sum(Mass.in_units('g')*velx.in_units('cm/s'), axis=0)/np.sum(Mass.in_units('g'), axis=0)
        V_CoM_y = np.sum(Mass.in_units('g')*vely.in_units('cm/s'), axis=0)/np.sum(Mass.in_units('g'), axis=0)
...
'''
if args.calculate_orbital_momentum == 'True':
    pickle_file = path + 'particle_data.pkl'
    file_open = open(pickle_file, 'rb')
    particle_data, sink_form_time, line_counter = pickle.load(file_open)
    file_open.close()
    ...
    #Calculate CoM
    CoM_x = np.sum(np.array(particle_data['mass'])*np.array(particle_data['posx']), axis=1)/np.sum(particle_data['mass'], axis=1)
    CoM_y = np.sum(np.array(particle_data['mass'])*np.array(particle_data['posy']), axis=1)/np.sum(particle_data['mass'], axis=1)
    CoM_z = np.sum(np.array(particle_data['mass'])*np.array(particle_data['posz']), axis=1)/np.sum(particle_data['mass'], axis=1)
    CoM = np.array([CoM_x, CoM_y, CoM_z]).T
    particle_data.update({'CoM': CoM.tolist()})
    print("Calculate Center of Mass position")
    
    #Calculate velocity of CoM
    V_CoM_x = np.sum(np.array(particle_data['mass'])*np.array(particle_data['velx']), axis=1)/np.sum(particle_data['mass'], axis=1)
    V_CoM_y = np.sum(np.array(particle_data['mass'])*np.array(particle_data['vely']), axis=1)/np.sum(particle_data['mass'], axis=1)
    V_CoM_z = np.sum(np.array(particle_data['mass'])*np.array(particle_data['velz']), axis=1)/np.sum(particle_data['mass'], axis=1)
    V_CoM = np.array([V_CoM_x, V_CoM_y, V_CoM_z]).T
    particle_data.update({'V_CoM': V_CoM.tolist()})
    print("Calculated Center of Mass velocity")
    
    L_x = particle_data['mass']*(((np.array(particle_data['velz']).T - np.array(V_CoM_z).T).T)*((np.array(particle_data['posy']).T-np.array(V_CoM_y).T).T)-((np.array(particle_data['vely']).T - np.array(V_CoM_y).T).T)*((np.array(particle_data['posz']).T-np.array(V_CoM_z).T).T))
    L_y = particle_data['mass']*(((np.array(particle_data['velz']).T - np.array(V_CoM_z).T).T)*((np.array(particle_data['posx']).T-np.array(V_CoM_x).T).T)-((np.array(particle_data['velx']).T - np.array(V_CoM_x).T).T)*((np.array(particle_data['posz']).T-np.array(V_CoM_z).T).T))
    L_z = particle_data['mass']*(((np.array(particle_data['vely']).T - np.array(V_CoM_y).T).T)*((np.array(particle_data['posx']).T-np.array(V_CoM_x).T).T)-((np.array(particle_data['velx']).T - np.array(V_CoM_x).T).T)*((np.array(particle_data['posy']).T-np.array(V_CoM_y).T).T))
    L_tot = np.sum(np.sqrt(L_x**2 + L_y**2 + L_z**2),axis=1)
    particle_data.update({'L_tot': L_tot.tolist()})
    print("Calculated orbital angular momentum")
    ...
    plt.clf()
    plt.semilogy(particle_data['time'], particle_data['L_tot'])
    plt.xlabel('Time (yr)')
    plt.ylabel('Total orbital angular momentum ($g\,cm^2/s$)')
    file_name = save_dir + 'Total_angular_momentum'
...
if args.phasefolded_accretion == 'True':
    ...
    #e_bins = [1.1, 0.7, 0.5, 0.4, 0.3, 0.1, 0.0]
    ...
    if use_e_bins:
        ...
        while e_bin_it < len(e_bins):
            ...
            if plot_it == 0:
                ...
                axes_dict[ax_label].set_ylim([0.0, 3.0])
            ...
            time_text = plt.text(0.05, axes_dict[ax_label].get_ylim()[1]-0.2, '$e$=['+str(e_bins[e_bin_it-1])+','+str(e_bins[e_bin_it])+'] using '+str(len(usable_periastrons))+' orbits', va="center", ha="left", color='k', fontsize=args.text_font)
        ...
        while rit < repeats:
            ...
            if fold_bool:
                ...
                median_acc_tot = np.median(averaged_total_accretion, axis=0)
                mean_acc_tot = np.mean(averaged_total_accretion, axis=0)
                std_acc_tot = np.std(averaged_total_accretion, axis=0, n=len)
                median_accretion_total.append(median_acc_tot)
                standard_deviation_total.append([median_acc_tot-(mean_acc_tot-std_acc_tot), (mean_acc_tot+std_acc_tot)-median_acc_tot])
                #plt.plot(phase_2, median_accretion[0].tolist()+median_accretion[0][1:].tolist(), ls='steps', alpha=0.5, label='Primary')
                #plt.plot(phase_2, median_accretion[1].tolist()+median_accretion[1][1:].tolist(), ls='steps', alpha=0.5, label='Secondary')
                ...
                yerr_2 = [standard_deviation[1][0].tolist()+standard_deviation[1][0].tolist()+standard_deviation[1][0].tolist(), standard_deviation[1][1].tolist()+standard_deviation[1][1].tolist()+standard_deviation[1][1].tolist()]
                yerr = [standard_deviation_total[0][0].tolist()+standard_deviation_total[0][0].tolist()+standard_deviation_total[0][0].tolist(), standard_deviation_total[0][1].tolist()+standard_deviation_total[0][1].tolist()+standard_deviation_total[0][1].tolist()]
                accretion_err.append(np.array(yerr)/np.array(long_median_total)) #This is the relative error
                axes_dict[ax_label].errorbar(phase_centers, np.array(long_median_accretion_1)*(1.e4), yerr=np.array(yerr_1)*(1.e4), ls='steps-mid', alpha=0.5, label='Primary')
                axes_dict[ax_label].errorbar(phase_centers, np.array(long_median_accretion_2)*(1.e4), yerr=np.array(yerr_2)*(1.e4), ls='steps-mid', alpha=0.5, label='Secondary')
                axes_dict[ax_label].errorbar(phase_centers, np.array(long_median_total)*(1.e4), yerr=np.array(yerr)*(1.e4), ls='steps-mid', label='Total')
    ...
    for file in files:
        ...
        for orbit in range(len(multiple_folds_normalised)):
            if args.beta_method == 1:
                ...
                #absolute error top
                max_err_abs_1 = accretion_err[orbit][0][max_ind_1:max_ind_2][usable_inds_max]*max_bin_values
                max_err_abs_2 = accretion_err[orbit][1][max_ind_1:max_ind_2][usable_inds_max]*max_bin_values
                max_err = [np.sqrt(np.sum(np.square(max_err_abs_1)))/len(max_bin_values)/max_mean, np.sqrt(np.sum(np.square(max_err_abs_2)))/len(max_bin_values)/max_mean]
                
                base_bin_values = multiple_folds_normalised[orbit][quiescent_ind_1:quiescent_ind_2]
                not_nan_inds = np.where((np.isnan(base_bin_values)==False) & (base_bin_values!=0.0))[0]
                base_mean = np.mean(base_bin_values[not_nan_inds])
                ...
                base_err_abs_1 = accretion_err[orbit][0][quiescent_ind_1:quiescent_ind_2][not_nan_inds]*base_bin_values[not_nan_inds]
                base_err_abs_2 = accretion_err[orbit][1][quiescent_ind_1:quiescent_ind_2][not_nan_inds]*base_bin_values[not_nan_inds]
                base_err = [np.sqrt(np.sum(np.square(base_err_abs_1)))/len(base_bin_values[not_nan_inds])/base_mean, np.sqrt(np.sum(np.square(base_err_abs_2)))/len(base_bin_values[not_nan_inds])/base_mean]
                ...
                base_bin_values = multiple_folds_normalised[orbit][quiescent_ind_1:quiescent_ind_2]
                not_nan_inds = np.where((np.isnan(base_bin_values)==False) & (base_bin_values!=0.0))[0]
                base_median = np.median(base_bin_values[not_nan_inds])
                #base_err = [np.mean(accretion_err[orbit][0][quiescent_ind_1:quiescent_ind_2]*base_bin_values)/base_median, np.mean(accretion_err[orbit][1][quiescent_ind_1:quiescent_ind_2]*base_bin_values)/base_median]
                base_err = [np.sqrt(np.sum(np.square(accretion_err[orbit][0][quiescent_ind_1:quiescent_ind_2][not_nan_inds]*base_bin_values[not_nan_inds])))/len(base_bin_values[not_nan_inds])/base_median, np.sqrt(np.sum(np.square(accretion_err[orbit][1][quiescent_ind_1:quiescent_ind_2][not_nan_inds]*base_bin_values[not_nan_inds])))/len(base_bin_values[not_nan_inds])/base_median]
...
if args.accretion_profile == 'True':
    ...
    if os.path.isfile(accretion_profile_pickle):
        ...
        smoothed_time = []
        smoothed_accretion = []
        smoothed_specific_L_spin = [[],[]]
        smoothed_specific_L_spin_rate = [[],[]]
        smoothed_specific_L_orb = [[],[]]
        smoothed_specific_L_orb_rate = [[],[]]
        particle_data_mass = yt.YTArray(particle_data['mass'], 'msun')
        particle_data_time = yt.YTArray(particle_data['time'], 'yr')
        #particle_data_quantity = np.array(particle_data['mdot'])
        particle_data_accretion = yt.YTArray(particle_data['mdot'], 'msun/yr')
        L_spin = yt.YTArray((np.sqrt(np.array(particle_data['anglx'])**2 + np.array(particle_data['angly'])**2 + np.array(particle_data['anglz'])**2)), 'g*cm**2/s')
        Specific_L_spin = L_spin/particle_data_mass.in_units('g')
        Specific_L_spin_rate = (Specific_L_spin.T[:,1:] - Specific_L_spin.T[:,:-1])/(particle_data_time.in_units('s')[1:]-particle_data_time.in_units('s')[:-1])
        Specific_L_spin_rate = yt.YTArray(np.vstack(([np.nan, np.nan],Specific_L_spin_rate.T.value)).T, 'cm**2/s**2')
        L_orb = yt.YTArray(np.array(particle_data['L_orb']), 'g*cm**2/s').T
        Specific_L_orb = L_orb/particle_data_mass.in_units('g')
        Specific_L_orb_rate = (Specific_L_orb.T[:,1:] - Specific_L_orb.T[:,:-1])/(particle_data_time.in_units('s')[1:]-particle_data_time.in_units('s')[:-1])
        Specific_L_orb_rate = yt.YTArray(np.vstack(([np.nan, np.nan],Specific_L_orb_rate.T.value)).T, 'cm**2/s**2')
        #particle_data_L_orb = np.array(particle_data['L_orb'])
        print("Smoothing accretion profile")
        for ind in range(len(particle_data_time)):
            smoothing_inds = np.where((particle_data_time.value > particle_data_time.value[ind]-window/2.)&(particle_data_time.value < particle_data_time.value[ind]+window/2.))[0]
            time = np.mean(particle_data_time[smoothing_inds])
            m_dot = np.mean(particle_data_accretion[smoothing_inds])
            L_spin_1 = np.mean(Specific_L_spin.T[0][smoothing_inds])
            L_spin_2 = np.mean(Specific_L_spin.T[1][smoothing_inds])
            L_spin_1_rate = np.mean(Specific_L_spin_rate[0][smoothing_inds])
            L_spin_2_rate = np.mean(Specific_L_spin_rate[1][smoothing_inds])
            L_orb_1 = np.mean(Specific_L_orb.T[0][smoothing_inds])
            L_orb_2 = np.mean(Specific_L_orb.T[1][smoothing_inds])
            L_orb_1_rate = np.mean(Specific_L_orb_rate[0][smoothing_inds])
            L_orb_2_rate = np.mean(Specific_L_orb_rate[1][smoothing_inds])
            smoothed_time.append(time)
            smoothed_accretion.append(m_dot)
            smoothed_specific_L_spin[0].append(L_spin_1)
            smoothed_specific_L_spin[1].append(L_spin_2)
            smoothed_specific_L_spin_rate[0].append(L_spin_1_rate)
            smoothed_specific_L_spin_rate[1].append(L_spin_2_rate)
            smoothed_specific_L_orb[0].append(L_orb_1)
            smoothed_specific_L_orb[1].append(L_orb_2)
            smoothed_specific_L_orb_rate[0].append(L_orb_1_rate)
            smoothed_specific_L_orb_rate[1].append(L_orb_2_rate)
        print("smoothed accretion profile")
        
        print("writing accretion profile pickle")
        file = open(accretion_profile_pickle, 'wb')
        pickle.dump((smoothed_time, smoothed_accretion, smoothed_specific_L_spin, smoothed_specific_L_spin_rate, smoothed_specific_L_orb, smoothed_specific_L_orb_rate), file)
        file.close()
    ...
    ax2.set_ylabel('Separation (AU)', fontsize=args.text_font)  # we already handled the x-label with ax1
