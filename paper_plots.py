    ...
    if args.slice_plot != False:
        if rank == 0:
            ...
            for rit in range(1,size):
                CW.send(usable_movie_files, dest=rit, tag=1)
        else:
            usable_movie_files = CW.recv(dest=1, tag=1)
        ...
        if args.yt_slice:
            ...
            if rank == rit:
                ...
                if args.produce_movie != False:
                    ...
                    print "on rank:", rank, "CREATED MOVIE FRAME No.", args.start_frame + fit, "OF", no_of_frames, "saved as:", save_image_name
            rit = rit +1
            if rit == size:
                rit = 0
