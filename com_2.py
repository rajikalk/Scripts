...
f = open('coms.txt','r+')
f.write("time, particles:x, y, z, gas:x, y, z, total:x, y, z, Error:x, y, z\n")
...
for pf in ts:
    ...
    '''
    x_err = pm1+(gl/2.) + pm2*(gl/2.)
    y_err = pm1+(gl/2.) + pm2*(gl/2.)
    z_err = pm1+(gl/2.) + pm2*(gl/2.)
    '''
    ...
    for x in range(dim):
        for y in range(dim):
            for z in range(dim):
                ...
                '''x_err = x_err + mass*(gl/2.)
                y_err = y_err + mass*(gl/2.)
                z_err = z_err + mass*(gl/2.)'''
