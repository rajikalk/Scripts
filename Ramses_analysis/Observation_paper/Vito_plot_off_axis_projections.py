...
def parse_inputs():
    ...
    parser.add_argument("-proj_vec", "--projection_vector", help="did you want to define a proj vector?", type=str, default=None)
    parser.add_argument("-north_vec", "--north_vector", help="did you want to define a north vector?", type=str, default=None)
    ...
    for fn_it in yt.parallel_objects(range(len(usable_files)), njobs=int(size/(8*4))): #8 projection and 4 fields.
        ...
        if make_pickle == True:
            ...
            if theta_pos_len<theta_neg_len:
                ...
                import pdb
                pdb.set_trace()
            if args.projection_vector == None:
                projection_vectors = []
                north_vectors = []
    ...
    if rank == rit:
        ...
        if make_frame:
            ...
            try:
                ...
                title_str = "proj vec:["+str(np.round(args_dict['proj_vector'][0]*100)/100)+ "," + str(np.round(args_dict['proj_vector'][1]*100)/100) +"," +str(np.round(args_dict['proj_vector'][2]*100)/100)+"], companion LOS pos:"+str(int(part_info['particle_position_z'][1]))+"AU"
