    ...
    for fn in yt.parallel_objects(usable_files, njobs=int(size/(6*para_div))):
        ...
        if make_pickle == True:
            ...
            if args.axis == 'xy':
                axis_ind = 2
                ax_str = 'z'
                left_corner = yt.YTArray([center_pos[0].value-(0.75*x_width), center_pos[1].value-(0.75*y_width), center_pos[2].value-(0.5*args.slice_thickness)], 'AU')
                right_corner = yt.YTArray([center_pos[0].value+(0.75*x_width), center_pos[1].value+(0.75*y_width), center_pos[2].value+(0.5*args.slice_thickness)], 'AU')
            ...
            if args.use_angular_momentum == 'False':
                ...
                if args.use_density_threshold != None:
                    ...
                    proj = yt.ProjectionPlot(ds, axis_ind, proj_axis_field, width=(x_width,'au'), weight_field=weight_field, method='sum', center=center_pos)
