...
plt.errorbar(np.array(masses)-105, Median_grads[0], yerr=Core_err, label='Core Fragmentation', color='b')
plt.errorbar(np.array(masses)-35, Median_grads[1], yerr=Delayed_core_err, label='Delayed Core Frag.', color='purple')
plt.errorbar(np.array(masses)+35, Median_grads[2], yerr=Capt_err, label='Dynamical Capture', color='r')
plt.errorbar(np.array(masses)+105, Median_grads[3], yerr=Other_err, label='Other', color='orange')
...
plt.ylim(top=1.5)
plt.savefig('inspiral_rate_comparison_medians.pdf', bbox_inches='tight')
