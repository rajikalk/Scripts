...
for grad_it in range(len(grad_pickles)):
    ...
    for Initial_mean_grad in mean_grads:
        ...
        axs_list[time_means_counter+1][grad_it].step(grad_bin_centers, np.append(grad_hist_core_mean_norm, np.array([grad_hist_core_mean_norm[-1]])), where='post', label="Core Fragmentation", linewidth=2, color='b', alpha=0.5, ls='-', width)
        ...
        axs_list[time_means_counter+1][grad_it].step(grad_bin_centers, np.append(grad_hist_core_delayed_mean_norm, np.array([grad_hist_core_delayed_mean_norm[-1]])), where='post', label="Delayed Core Fragmentation", linewidth=2, color='purple', alpha=0.5, ls='--')
        ...
        axs_list[time_means_counter+1][grad_it].step(grad_bin_centers, np.append(grad_hist_capt_mean_norm, np.array([grad_hist_capt_mean_norm[-1]])), where='post', label="Dynamical Capture", linewidth=2, color='red', alpha=0.5, ls='-.')
        ...
        axs_list[time_means_counter+1][grad_it].step(grad_bin_centers, np.append(grad_hist_misc_mean_norm, np.array([grad_hist_misc_mean_norm[-1]])), where='post', label="Other", linewidth=2, color='orange', alpha=0.5, ls=':')
