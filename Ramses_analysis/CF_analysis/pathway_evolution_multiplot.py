...
for grad_it in range(len(grad_pickles)):
    ...
    for Initial_mean_grad in mean_grads:
        ...
        if time_means[time_means_counter] == 10000:
            ...
            popt, pcov = curve_fit(Gaussian, bin_centres, (grad_hist_core_mean_norm[:-2]), [scale_guess, mean_guess, std_guess], bounds=([0.0, bin_centres[0], 0.0], [1.0, bin_centres[-1], (bin_centres[0]-bin_centres[-1])]))
        ...
        if time_means[time_means_counter] == 10000:
            ...
            popt, pcov = curve_fit(Gaussian, bin_centres, (grad_hist_core_delayed_mean_norm[:-2]), [scale_guess, mean_guess, std_guess], bounds=([0.0, bin_centres[0], 0.0], [1.0, bin_centres[-1], (bin_centres[0]-bin_centres[-1])]))
            x_fit = np.linspace(0, len(grad_hist_core))
            fit = Gaussian(x_fit, *popt)
        ...
        if time_means[time_means_counter] == 10000:
            ...
            try:
                popt, pcov = curve_fit(Gaussian, bin_centres, (grad_hist_capt_mean_norm[:-2]), [scale_guess, mean_guess, std_guess], bounds=([0.0, bin_centres[0], 0.0], [1.0, bin_centres[-1], (bin_centres[0]-bin_centres[-1])]))
                x_fit = np.linspace(0, len(grad_hist_core))
                fit = Gaussian(x_fit, *popt)
        ...
        if time_means[time_means_counter] == 10000:
            ...
            try:
                popt, pcov = curve_fit(Gaussian, bin_centres, (grad_hist_misc_mean_norm[:-2]), [scale_guess, mean_guess, std_guess], bounds=([0.0, bin_centres[0], 0.0], [1.0, bin_centres[-1], (bin_centres[0]-bin_centres[-1])]))
                x_fit = np.linspace(0, len(grad_hist_core))
                fit = Gaussian(x_fit, *popt)
