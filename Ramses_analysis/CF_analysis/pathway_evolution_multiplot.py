...
for grad_it in range(len(grad_pickles)):
    ...
    for Initial_mean_grad in mean_grads:
        ...
        if time_means[time_means_counter] == 10000:
            ...
            try:
                popt, pcov = curve_fit(Skewed_Gaussian, bin_centres[::-1], (grad_hist_core_mean_norm[:-2][::-1]), [scale_guess, mean_guess, std_guess, skew_guess], bounds=([0.0, bin_centres[-1], 0.0, -np.inf], [1.0, bin_centres[0], (bin_centres[0]-bin_centres[-1]), np.inf]))
        ...
        if time_means[time_means_counter] == 10000:
            ...
            skew_guess = (3*(mean_guess - median_guess))/std_guess
            try:
                popt, pcov = curve_fit(Gaussian, bin_centres[::-1], (grad_hist_core_delayed_mean_norm[:-2][::-1]), [scale_guess, mean_guess, std_guess, skew_guess], bounds=([0.0, bin_centres[-1], 0.0, -np.inf], [1.0, bin_centres[0], (bin_centres[0]-bin_centres[-1]), np.inf]))
                x_fit = np.linspace(0, len(grad_hist_core))
                fit = Gaussian(np.linspace(bin_centres[0], bin_centres[-1]), *popt)
        ...
        if time_means[time_means_counter] == 10000:
            ...
            skew_guess = (3*(mean_guess - median_guess))/std_guess
            try:
                popt, pcov = curve_fit(Gaussian, bin_centres[::-1], (grad_hist_capt_mean_norm[:-2][::-1]), [scale_guess, mean_guess, std_guess, skew_guess], bounds=([0.0, bin_centres[-1], 0.0, -np.inf], [1.0, bin_centres[0], (bin_centres[0]-bin_centres[-1]), np.inf]))
                x_fit = np.linspace(0, len(grad_hist_core))
                fit = Gaussian(np.linspace(bin_centres[0], bin_centres[-1]), *popt)
        ...
        if time_means[time_means_counter] == 10000:
            ...
            skew_guess = (3*(mean_guess - median_guess))/std_guess
            try:
                popt, pcov = curve_fit(Gaussian, bin_centres[::-1], (grad_hist_misc_mean_norm[:-2][::-1]), [scale_guess, mean_guess, std_guess, skew_guess], bounds=([0.0, bin_centres[-1], 0.0, -np.inf], [1.0, bin_centres[0], (bin_centres[0]-bin_centres[-1]), np.inf]))
                x_fit = np.linspace(0, len(grad_hist_core))
                fit = Gaussian(np.linspace(bin_centres[0], bin_centres[-1]), *popt)
