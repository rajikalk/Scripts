...
for grad_it in range(len(grad_pickles)):
    ...
    #This section only selects systems that are inspiralling
    core_inspiral_inds = np.where(np.array(Initial_gradients[0]) < 0)[0]
    delayed_core_inspiral_inds = np.where(np.array(Initial_gradients[1]) < 0)[0]
    capt_inspiral_inds = np.where(np.array(Initial_gradients[2]) < 0)[0]
    other_inspiral_inds = np.where(np.array(Initial_gradients[3]) < 0)[0]
...
axs[0].errorbar(np.array(masses)-130, 10**(np.array(Grad_over_sep_median[0]))*1000, yerr=Core_err, label='Bound core frag.', color='b', linestyle=':')
axs[0].errorbar(np.array(masses), 10**(np.array(Grad_over_sep_median[1]))*1000, yerr=Delayed_core_err, label='Unbound core frag.', color='purple', linestyle=':')
axs[0].errorbar(np.array(masses)+130, 10**(np.array(Grad_over_sep_median[2]))*1000, yerr=Capt_err, label='Dynamical capture', color='r', linestyle=':')

axs[1].errorbar(np.array(masses)-130, 10**(np.array(Grad_over_sep_median_10000[0]))*10000, yerr=Core_err_10000, label='Bound core frag.', color='b', linestyle=':')
axs[1].errorbar(np.array(masses), 10**(np.array(Grad_over_sep_median_10000[1]))*10000, yerr=Delayed_core_err_10000, label='Unbound core frag.', color='purple', linestyle=':')
axs[1].errorbar(np.array(masses)+130, 10**(np.array(Grad_over_sep_median_10000[2]))*10000, yerr=Capt_err_10000, label='Dynamical capture', color='r', linestyle=':')
