...
file_open = open("/groups/astro/rlk/rlk/Analysis_plots/Superplot_pickles_entire_sim/Perseus_CDF_full.pkl", "rb")
...
for SFE_it in range(len(SFE[SFE_1_ind:])):
...
#plt.ylim(top=1000)
#plt.ylim([np.argmin(reduced_chi_square_tobin), np.argmax(reduced_chi_square_tobin)])
plt.savefig(args.save_directory + "KS_test_alpha_"+str(alpha)+"_smoothed_120.pdf", format='pdf', bbox_inches='tight', pad_inches = 0.02)

file = open(args.save_directory + 'chi_squared_fit_120.pkl', 'wb')
