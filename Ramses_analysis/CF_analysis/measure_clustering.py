...
for time_it in time_it_range[-2:-1]:
    ...
    if rank == rit:
        ...
        if N_D > 1:
            ...
            DD_rel_err = np.nan_to_num(DD_rel_err)
            TPCF = DD/RR
            ...
            popt1, pcov1 = curve_fit(line, sep_centers[:power_law_break_ind], np.log10(TPCF_frac[:power_law_break_ind]), p0=[grad_guess, y_intercept_guess], sigma=np.log10(TPCF_err/(TPCF_frac+1e-5))[:power_law_break_ind], absolute_sigma=True)
