...
if args.update_pickle == 'True':
    ...
    if os.path.isfile('nearby_accretion.pkl'):
        try:
            file_open = open(save_dir+'nearby_accretion.pkl', 'rb')
            nearby_accretion, counter, sink_ind, sink_form_time = pickle.load(file_open)
            file_open.close()
            counter = int(counter)
            ...
            os.system('cp '+save_dir+'nearby_accretion_tmp.pkl '+save_dir+'nearby_accretion.pkl ')
            file_open = open(save_dir+'nearby_accretion.pkl', 'rb')
            nearby_accretion, counter, sink_ind, sink_form_time = pickle.load(file_open)
            file_open.close()
            counter = int(counter)
        ...
        nearby_accretion = {}
        nearby_accretion.update({'particle_tag':[]})
        nearby_accretion.update({'time':[]})
        nearby_accretion.update({'mass':[]})
        nearby_accretion.update({'mdot':[]})
    ...
    if updating == True:
        ...
        for sink_data in loaded_sink_data:
            ...
            if np.remainder(counter, 1000) == 0:
                try:
                    os.remove(save_dir+'nearby_accretion.pkl')
                except:
...
m_dot = yt.YTArray(nearby_accretion['mdot']).in_units('g/s')
mass = yt.YTArray(nearby_accretion['mass']).in_units('g')
...
plt.clf()
for part_it in range(len(nearby_accretion['particle_tag'])):
    plt.plot(nearby_accretion['time'], Mag.T[part_it], label="Sink "+str(nearby_accretion['particle_tag'][part_it]+1))
...
plt.clf()
for part_it in range(len(nearby_accretion['particle_tag'])):
    plt.semilogy(nearby_accretion['time'], L_tot.T[part_it], label="Sink "+str(nearby_accretion['particle_tag'][part_it]+1))
plt.xlabel('Time (yr)')
plt.xlim()
plt.legend()
...
plt.clf()
for part_it in range(len(nearby_accretion['particle_tag'])):
    plt.semilogy(nearby_accretion['time'], np.array(nearby_accretion['mdot']).T[part_it], label="Sink "+str(nearby_accretion['particle_tag'][part_it]+1))
plt.xlabel('Time (yr)')
plt.xlim()
plt.legend()
...
plt.clf()
for part_it in range(len(nearby_accretion['particle_tag'])):
    plt.plot(nearby_accretion['time'], np.array(nearby_accretion['mass']).T[part_it], label="Sink "+str(nearby_accretion['particle_tag'][part_it]+1))
plt.xlabel('Time (yr)')
plt.xlim()
plt.legend()
...
plt.clf()
plt.semilogy(nearby_accretion['time'], nearby_accretion['separation'])
...
if sink_ind == 45:
    start_time = 3500
    start_ind = np.argmin(abs(np.array(nearby_accretion['time']) - start_time))
    plt.clf()
    plt.semilogy(nearby_accretion['time'][start_ind:], nearby_accretion['mdot'][start_ind:])
    plt.xlabel('Time (yr)')
    plt.xlim()
