...
if args.make_pickle_files == "True":
    ...
    for fn in yt.parallel_objects(usable_files, njobs=size):
        ...
        if make_pickle:
            ...
            #    import pdb
            #    pdb.set_trace()
            accreted_inds_burst = np.in1d(particle_identity.value, accreted_ids_burst).nonzero()[0]
            accrete_inds_other = np.in1d(particle_identity.value, accreted_ids_other).nonzero()[0]
            not_accreted_inds = np.in1d(particle_identity.value, not_accreted_ids).nonzero()[0]
            del particle_identity
            gc.collect()
