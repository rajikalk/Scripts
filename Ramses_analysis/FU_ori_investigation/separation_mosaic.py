...
while fit < no_frames:
    ...
    if rank == rit:
        ...
        if os.path.isfile(frame_name) == False and os.path.isfile(args.input_dir+'/XY/movie_frame_' + ("%06d" % fit) +'.pkl') and os.path.isfile(args.input_dir+'/XZ/movie_frame_' + ("%06d" % fit) +'.pkl') and os.path.isfile(args.input_dir+'/YZ/movie_frame_' + ("%06d" % fit) +'.pkl'):
            ...
            if os.path.isfile(tracer_file):
                ax1.scatter(tracer_data['rely'], tracer_data['relz'], marker='.', markersize=1, color='magenta', edgecolor=None)
            #del image
            gc.collect()
            ...
            if os.path.isfile(tracer_file):
                ax1.scatter(tracer_data['relx'], tracer_data['relz'], marker='.', markersize=1, color='magenta', edgecolor=None)
            #del image
            gc.collect()
            ...
            if os.path.isfile(tracer_file):
                ax1.scatter(tracer_data['relx'], tracer_data['rely'], marker='.', markersize=1, color='magenta', edgecolor=None)
            #del image
            gc.collect()
