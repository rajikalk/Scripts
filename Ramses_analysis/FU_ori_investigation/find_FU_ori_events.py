...
if rank == 0:
    ...
    for plot_it in range(len(top_clean[:10])):
        ...
        ax2 = ax1.twinx()
        ax1.plot(useable_times/10000, scaled_L, label="Scaled Luminosity", color='b')
        ax1.plot(useable_times/10000, cor[:len(useable_times)]/100., label="Correlation", color='r')
                            
        ax2.plot(useable_times/10000, useable_L, color='b')

        if plot_it >4:
            ax1.set_xlabel('Time (10$^4$yr)', fontsize=font_size, labelpad=-1)
        ...
        useable_times = useable_times/10000
