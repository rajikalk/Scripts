...
for event_it in event_IDS:
    ...
    lns_all = []
    lns1 = ax.semilogy(particle_data['time'][start_ind:end_ind], particle_data['mdot'].T[1][start_ind:end_ind], color='k', ls='-', label="Accretion rate")
    lns_all.append(lns1)
    Radii = [1, 2, 3, 4, 5, 6]
    ...
    for rad in Radii:
        ...
        if plot == True:
            ...
            lns3 = ax.semilogy(save_dict["Time"], BHL_mean, color=colours[Radii.index(rad)], ls=':', label="r = "+str(rad)+"AU")
            lns_all.append(lns3)
            #plt.ylim([np.min(particle_data['mdot'].T[1][start_ind:end_ind]), np.max(particle_data['mdot'].T[1][start_ind:end_ind])])
            ax.fill_between(save_dict["Time"], save_dict["BHL_Acc_acc_low"], save_dict["BHL_Acc_acc_high"], color=colours[Radii.index(rad)], alpha=0.1)
    axes_1_twin = ax.twinx()
