...
#------------------------------------------------------
plot_times = [3000, 4800, 6100, 6950, 7600, 8100, 8450, 8700]
...
stdvel = 3
n_frames = 8
cbar_lims = [1.e-16, 5.e-14]# [1.e-16, 5.e-14]
start_time = plot_times[0]
end_time = plot_times[-1]


plt.clf()
fig = plt.figure(figsize=(two_col_width, 0.83*two_col_width))
fig.subplots_adjust(left=0, right=1, bottom=0, top=1)
G = gridspec.GridSpec(3,int(n_frames/2))#,height_ratios=[1.5, 2, 2])
...
for plot_time in plot_times:
    ...
    if os.path.isfile(movie_plot_pickle) == False:
        #Make movie frame
        cmd = ['python', '/home/100/rlk100/Scripts/Ramses_analysis/movie_script.py', '/home/100/rlk100/gdata/RAMSES/Zoom-in_CPH_sims/Sink_45/Level_19/Level_20/data/', './', '-sink', '45', '-pt', str(plot_time), '-at', 'True', '-pvl', 'True',  '-ax', 'xz', '-al', '15', '-tf', '12', '-stdv', str(stdvel), '-thickness', '30', '-use_gas', 'False', '-ic', '1', '-update_alim', 'True', '-frames_only', 'False', '-apm', 'True', '-vaf', '15', '-res', '1028', '-wf', 'Density']
    ...
    axes_1.scatter(particle_data['time'][plot_time_ind], particle_data['separation'][plot_time_ind], marker='o', s=50, color='g', alpha=0.5)
    ...
    if plot_it == n_frames-1:
        ...
        cbar_ax = fig.add_axes([1, 0.005, 0.015, 0.645])
    ...
    mym.annotate_particles(ax, part_info['particle_position'], part_info['accretion_rad'], limits=[xlim, ylim], annotate_field=part_info['particle_mass'], particle_tags=part_info['particle_tag'], zorder=7, standard_vel=0.5, annotate_velocity=True, particle_velocity=part_info['particle_velocity'])
