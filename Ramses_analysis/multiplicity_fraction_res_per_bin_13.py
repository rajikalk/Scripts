...
if rank == 0:
    ...
    if len(pickle_files) > 0:
        ...
        try:
            ...
            Times, CF_arrays, N_sys_total, Sink_Luminosities_full, Sink_Accretion_full = pickle.load(file)
            file.close()
        except:
...
if args.match_method == 1:
    ...
    if args.use_t_spread == 'True':
        ...
        SFE_t_ff_min = global_data['time'].T[0][SFE_min_ind]
        SFE_max_ind = np.argmin(np.abs((SFE_value+SFE_spread)-SFE))
        SFE_t_ff_max = global_data['time'].T[0][SFE_max_ind]
        time_bounds = [SFE_t_ff_min,SFE_t_ff_max]
    if rank == 0:
        plt.clf()
        plt.plot(global_data['time'].T[0], SFE)
    ...
    plt.clf()
    plt.plot(global_data['time'].T[0], total_stars)
    plt.xlabel("Time (t$_{ff}$)")
    ...
    if args.use_t_spread == 'True':
        ...
        if rank == 0:
            plt.clf()
            plt.plot(global_data['time'].T[0], total_stars)
            plt.plot(global_data['time'].T[0], N_vis_array)
            plt.xlabel("Time (t$_{ff}$)")
            plt.ylabel("N$_{stars}$")
    ...
    if rank == 0:
        plt.clf()
        plt.plot(global_data['time'].T[0], M_tot)
        plt.xlabel("Time (t$_{ff}$)")
