...
if len(usable_files) == 1:
    ...
    njobs = int(size/8*4)
for fn_it in yt.parallel_objects(range(len(usable_files)), njobs=njobs): #8 projection and 4 fields.
    ...
    if make_pickle == True:
        ...
        for proj_it in yt.parallel_objects(range(len(projection_vectors)), njobs=8):# range(len(projection_vectors)):
            if True in np.isnan(projection_vectors[proj_it]):
                ...
                print("Calculating projection with normal", proj_vector_unit, "on rank", rank)
                proj = yt.OffAxisProjectionPlot(ds, proj_vector_unit, field_list, width=(2*args.ax_lim, 'AU'), weight_field=weight_field, method='integrate', center=(center_pos.value, 'AU'), depth=(args.slice_thickness, 'AU'), north_vector=north_unit)
