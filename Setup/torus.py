...
datadir_particles = '/home/science/staff/reggie/Simulation/Hot_fb_0.5k/DD0000/CE0000.cpu0000'
rootdir = '/home/science/staff/reggie/Simulation/Hot_fb_0.5k/DD0000/CE0000.hierarchy.hdf5'
...
potparticle1=(-1*pf['GravitationalConstant'])/(4*3.14)*particle_mass[0]*cellvolume/((x-particle_position_x[0])**2+(y-particle_position_y[0])**2+(z-particle_position_z[0])**2)**0.5
potparticle2=(-1*pf['GravitationalConstant'])/(4*3.14)*particle_mass[1]*cellvolume/((x-particle_position_x[1])**2+(y-particle_position_y[1])**2+(z-particle_position_z[1])**2)**0.5
pot=potparticle1+potparticle2
# total_energy = total_energy/500.0 # about 200 K
#total_energy = total_energy/5.0 # about 2*17543 K
...
#      if ((x[i*256**2]-0.5)**2+(y[j*256]-0.5)**2+(z[k]-0.5)**2+Rtor**2-torr**2)**2-4*Rtor**2*((x[i*256**2]-0.5)**2+(y[j*256]-0.5)**2) < 0.0:
        ...
        radius = ((x[i*256**2]-0.5)**2+(y[j*256]-0.5)**2)**0.5
        if (radius == 0.0): 
          radius = 0.001
# Kepler velocity is sqrt(-pot):
        yvelocity[k][j][i] = (x[i*256**2]-0.5)/radius * (-pot[i*256**2+j*256+boxsize/2])**0.5*(1.0/2.0)
        xvelocity[k][j][i] = -(y[j*256]-0.5)/radius * (-pot[i*256**2+j*256+boxsize/2])**0.5*(1.0/2.0)
#      else:
...
f=h5py.File('/home/science/staff/reggie/Simulation/Hot_fb_0.5k/DD0000/CE0000.cpu0000')
