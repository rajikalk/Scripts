...
with open('/home/100/rlk100/RV_standard_list.csv', 'rU') as f:
    ...
    for row in reader:
        if header != 0:
            RV_standard_info[row[2]] = (float(row[5]), float(row[6]), float(row[7]))
        else:
            header = 1
    ...
    for row in reader:
        if header != 0:
            ...
            if row[0] != 'UCAC4-1253626396':
                ...
                if row[11] == 'FALSE' or row[13] == 'False':
                    SB1_flag.append(False)
                else:
                    SB1_flag.append(True)
                if row[12] == 'FALSE' or row[14] == 'False':
...
if args.calculate_bayes == 'False':
    ...
    gamma_prior = P_gamma #1.0 #A constant for now.
    p_gamma_US = gamma_prior
    p_gamma_UCL = gamma_prior

    #US
    for bayes in all_bayes[0]:
        p_gamma_US *= gamma*bayes + (1.0-gamma)
    p_gamma_US = p_gamma_US/np.max(p_gamma_US)
    ...
    max_US = gamma[np.argmax(p_gamma_US_norm)]
    ...
    for x_gamma in range(len(gamma)-2)[1:]:
        ...
        if delta > prev_delta:
            ...
            break
        prev_delta = delta
    US_pos_error = US_84th - max_US
    US_neg_error = max_US - US_16th
    ...
    #p_gamma_UCL = np.nan_to_num(p_gamma_UCL)
    p_gamma_UCL = p_gamma_UCL/np.max(p_gamma_UCL)
    ...
    max_UCL = gamma[np.argmax(p_gamma_UCL_norm)]
    ...
    for x_gamma in range(len(gamma)-2)[1:]:
        ...
        if delta > prev_delta:
            ...
            break
        prev_delta = delta
    UCL_pos_error = UCL_84th - max_UCL
    UCL_neg_error = max_UCL - UCL_16th
    ...
    UCL_inds = np.where((gamma > (max_UCL - UCL_neg_error))&(gamma < (max_UCL + UCL_pos_error)))[0]
    plt.clf()
    plt.axvline(x=0.12, color='k')
    #plt.fill_between([11,13], np.zeros(len(gamma)), np.ones(len(gamma)), where=y >= y1, facecolor='green', interpolate=True, alpha=0.5)
    plt.axvspan(0.11, 0.14, alpha=0.5, color='grey')
    plt.plot(gamma, p_gamma_US_norm, label='Upper Scorpius', color='b')
    plt.fill_between(gamma[US_inds], np.zeros(len(gamma[US_inds])), p_gamma_US_norm[US_inds], facecolor='blue', alpha=0.5, edgecolors='none')
    plt.plot(gamma, p_gamma_UCL_norm, label='Upper Centaurus-Lupus', color='g')
    plt.fill_between(gamma[UCL_inds], np.zeros(len(gamma[UCL_inds])), p_gamma_UCL_norm[UCL_inds], facecolor='green', alpha=0.5, edgecolors='none')
    plt.legend(loc='best')
    plt.xlim([0.0, 0.5])
    ...
    plt.savefig('Composite_fractions' + args.image_suffix + '.eps', bbox_inches='tight', pad_inches = 0.02)
    plt.savefig('Composite_fractions' + args.image_suffix + '.pdf', bbox_inches='tight', pad_inches = 0.02)
    ...
    US_per_SpT_err = []
    UCL_per_SpT_err = []
    gamma_prior = P_gamma #A constant for now.
    p_gamma_US_F = gamma_prior
    p_gamma_UCL_F = gamma_prior
    p_gamma_US_G = gamma_prior
    p_gamma_UCL_G = gamma_prior
    p_gamma_US_K = gamma_prior
    p_gamma_UCL_K = gamma_prior
    p_gamma_US_M = gamma_prior
    p_gamma_UCL_M = gamma_prior
