...
with open('Table_1.csv', 'rU') as f:
    ...
    for row in reader:
        if header != 0:
            if float(row[9]) > 1000:
                line = row[0] + '&' + row[1] + '&' + row[2] + '&' + row[3] + '&' + row[4] + '&' + row[6] + '&' + row[7] + '&' + row[8] + '&' + str((np.round(float(row[9])*100.))/100.) + '&' + '$>10^3$' + '(' + row[11] + ')\\'
            else:
                line = row[0] + '&' + row[1] + '&' + row[2] + '&' + row[3] + '&' + row[4] + '&' + row[6] + '&' + row[7] + '&' + row[8] + '&' + str((np.round(float(row[9])*100.))/100.) + '&' + str((np.round(float(row[10])*100))/100.) + '(' + row[11] + ')\\'
...
f.write('            Object & Region & RA & DEC & P(M) & Disk? & $N_{Obs}$ & Temp. SpT. & $\Delta v_r$ & Bayes\' Factor(binary?)\\')
f.write('\\')
f.write('\n')
f.write('                \\hline\n')
